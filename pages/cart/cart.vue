<template>
	<view class="page">
		...
		<view class="recharge-btn" >
			...
			<view class="btn2" v-else-if="timeshow == 2">消费未达标，联系客服</view>
		</view>
		<!-- tabbar -->
...
<script>
	...
	import { createtixian,findCustomData,findConfig,orderList2 } from "../../api/index";
	...
	export default {
		...
		methods:{
			async checktime(){
				let _this = this
				//统计消费金额
				let  orderlist  = await orderList2()
				orderlist = orderlist.result
				
				let sum = 0;
				        for (var i = 0; i < orderlist.length; i++) {
				          sum += orderlist[i].productPrice;
				        }
				
				
				let userinfo = await findCustomData()
				...
				await findConfig().then((res)=>{
					...
					console.log('累积消费：',sum,'累积充值：',userinfo.realAccount,'累积赠送：', userinfo.zengAccount)
					let countmoney = userinfo.realAccount + userinfo.zengAccount
					if(sum < countmoney ){
						_this.timeshow = 2
