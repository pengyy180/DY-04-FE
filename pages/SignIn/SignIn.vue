<template>
	<view class="page" >
		...
		<view class="user-list">
			...
			<view class="list" @click="onPayUp">
				<view class="title" @click.stop="showbanknumberchange">
					<text>银行卡号</text>
				</view>
				
				<view class="more-content" v-if="showbanknumber == false">
					<text class="content">{{userinfo.bankNum==null?'请输入银行卡号': userinfo.bankNum | hideMiddle}}</text>	
				</view>
				<view class="more-content" v-if="showbanknumber == true">
	...
	export default {
		...
		data() {
			...
			return {
				...
				bankname: '',
				nickname: '罗希纳斯',
				banknumber:'11111111111111111111',
				userinfo:'',
				bankCustomName:'',	
				bankNameDesc:'',
				bankNum:[],
		...
		methods:{
			showbanknumberchange(){
					if(this.showbanknumber == false){
						this.showbanknumber = true
					}else{
						this.showbanknumber = false
					}
			},
			bankCard(value){
			...
			KeyInfo(val){
				...
				// 判断是否删除
				if(val.keyCode === 8){
					this.bankNum.splice(val.index+1,1)
				}else{
					this.bankNum.push(val.key);
				}
				this.userinfo.bankNum = this.bankNum.join('')
				console.log()
				this.$forceUpdate();
			},
			getBank(){//银行列表
